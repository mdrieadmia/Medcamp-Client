...
const ManageCampTable = ({camps}) => {
    const columns = [
        ...
        {
            name : 'Actions',
            selector : (camp) => <div className='py-5 flex justify-center gap-3 items-center text-lg'>
                <Link to={`/dashboard/camp/update/${camp._id}`} className='text-green-500 font-bold mr-2'><BiSolidEdit className='text-xl'/></Link>
                <button className='text-red-500 font-bold ml-2'> <ImBin/> </button> 
                </div> ,
            sortable : true           
        },
    ]
